...
def sentiment_distribution(mapped_rows: List[Dict[str, Any]]) -> pd.DataFrame:
...
def generate_ai_summary_together_cached(analytics_str: str) -> str:
    import requests
    import os
    import json
...
with st.spinner("Insights are loading..."):
...
with main_col:
    ...
    if px:
        df_compare = pd.DataFrame({
            "Stage": ["Spark", "Growth", "Horizon"],
            "Score": [social_spark, social_growth, social_horizon]
        })
    ...
    with s3:
        ...
        df_cultural = cultural_stacked_data(filtered_rows)

        if px and not df_cultural.empty:
            fig = px.bar(
                df_cultural,
                x="indicator",
                y="percentage",
        ...
        st.caption("Shows distribution of Positive, Neutral, and Negative responses across cultural outcomes.")
    ...
    st.markdown('<div class="side-card" style="margin-top:16px;">', unsafe_allow_html=True)
    df_sentiment = sentiment_distribution(filtered_rows)
    if px:
         fig = px.bar(
             df_sentiment,
             x="count",
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.caption("Overall emotional tone of participant feedback.")
